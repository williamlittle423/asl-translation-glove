    ...
    for label_idx, label in enumerate(labels):
        ...
        for input_idx in range(N_training):
            ...
            for sensor in range(5):
                ...
                mean_accel.extend([mean_accel_x, mean_accel_y, mean_accel_z])
                std_accel.extend([std_accel_x, std_accel_y, std_accel_z])
                ...
                mean_gyro.extend([mean_gyro_x, mean_gyro_y, mean_gyro_z])
                std_gyro.extend([std_gyro_x, std_gyro_y, std_gyro_z])

            # Prepare the row data
            row = [label, input_idx + 1]
            for i in range(5):  # For each sensor
                row.extend([
                    mean_accel[i*3 + 0], std_accel[i*3 + 0],  # Accel X
                    mean_accel[i*3 + 1], std_accel[i*3 + 1],  # Accel Y
                    mean_accel[i*3 + 2], std_accel[i*3 + 2],  # Accel Z
                    mean_gyro[i*3 + 0], std_gyro[i*3 + 0],    # Gyro X
                    mean_gyro[i*3 + 1], std_gyro[i*3 + 1],    # Gyro Y
                    mean_gyro[i*3 + 2], std_gyro[i*3 + 2],    # Gyro Z
                ])
