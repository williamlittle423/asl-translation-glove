...
parser.add_argument("--model_path", type=str, default='asl_mlp_model.pth', help="Path to the trained MLP model (.pth file)")
parser.add_argument("--scaler_path", type=str, default='asl_scaler.save', help="Path to the saved StandardScaler (.save file)")
parser.add_argument("--letters", type=str, nargs='+', help="List of letters to classify (e.g., A B C D)", default=['A', 'B', 'C', 'D'])
...
parser.add_argument("--num_classes", type=int, default=4, help="Number of classes (letters)")
...
parser.add_argument("--threshold", type=float, default=0.8, help="Probability threshold for prediction (default 80%)")
...
# Step 6: Define I2C Communication Functions with Error Handling
...
def predict_letter(feature_vector):
...
# Step 9: Main Inference Loop
def main():
    print("\n--- ASL Letter Inference Started ---\n")
    print("Instructions:")
    print("1. Press Enter to perform an ASL letter gesture.")
    print("2. The system will collect data and attempt to predict the letter.")
    print("3. Prediction will be displayed if confidence > 80%.")
    print("4. Press Ctrl+C to exit.\n")
    
    try:
        while True:
            input("Press Enter to perform an ASL letter gesture...")
            feature_vector = collect_features()
            if feature_vector is None:
                print("Data collection failed. Please try again.\n")
                continue
            predicted_class, max_prob, probabilities = predict_letter(feature_vector)
